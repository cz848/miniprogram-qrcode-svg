import { QRErrorCorrectLevel } from './constants.js';
...
export default class QRCode {
  ...
  setOptions(options) {
    ...
    this.options = {
      ...
      // The following options are for SVG
      width: 256,
      height: 256,
      padding: 0,
      color: '#000',
  ...
  init(content) {
    const { padding, width, height, ecl, typeNumber } = this.options;
  ...
  toSVG(opt) {
    ...
    const defs = predefined ? `${indent}<defs><path id="qrmodule" d="M0 0 h${xsize} v${ysize} H0 z" fill="${color}" shape-rendering="crispEdges"/></defs>${eol}` : '';
